...
#include "get_Position_Ctrl_mmy.hpp"
...
static bool Msg245_EXTENDED_SYS_STATE( uint8_t port , mavlink_message_t* msg_sd )//takeoff and land status only valid in M35.
{
	uint8_t landed_state;
	bool inFlight;
	get_is_inFlight(&inFlight);
	bool takingoff;
	get_is_takingoff(&takingoff);
	bool landing;
	get_is_landing(&landing);	
	if(!inFlight)landed_state=MAV_LANDED_STATE_ON_GROUND;
	else if(takingoff)landed_state=MAV_LANDED_STATE_TAKEOFF;
...
{	
	 vector3<double> vel;
	 get_Position_Ctrl_mmy(&vel);
   //get_VelocityENU_Ctrl_mmy(&vel);	
	 get_TF_mini_Z( &my_info );
	mavlink_msg_global_position_int_pack_chan( 
		get_CommulinkSysId() ,	//system id
		get_CommulinkCompId() ,	//component id
		port	,	//chan
...
static bool Msg250_DEBUG_VECT( uint8_t Port_index , mavlink_message_t* msg )
{
		 vector3<double> vel;
	 get_Position_Ctrl_mmy(&vel);
	//get_VelocityENU_Ctrl_mmy(&vel);
	static bool pscsend=true;
    bool is_RC_Control;
	Position_ControlMode pscmode,altmode;
	get_Position_ControlMode(&pscmode);
	get_Altitude_ControlMode(&altmode);
  get_is_RC_Control(&is_RC_Control);
	mavlink_msg_debug_vect_pack_chan(
			...
			msg,
			"PSCMODE",
			TIME::get_System_Run_Time() * 1000 ,	//latitude
			pscmode,
			altmode,
			is_RC_Control
		);
