...
//	  static double iniYaw;
		...
		if(!vision_pos_present)
		{
			    PositionSensorRegister( default_vision_pos_sensor_index, \
                            ...
                            0.1,//XY信任度
			                      0 );
					vision_pos_present=true;
				 PositionSensorRegister( default_vision_height_sensor_index_slam , \
		...
		else
		{
			  ...
			  vision_z = pos_vision.z / 100.0;
			  //pos_vision.x=(msg_rd->y)*100;
			  //pos_vision.y=(msg_rd->x)*100;
				...
				PositionSensorUpdatePosition(default_vision_pos_sensor_index,pos_vision,true);
			  PositionSensorUpdatePosition(default_vision_height_sensor_index_slam,pos_vision,true);

		}
}

static void Msg103_VISION_SPEED_ESTIMATE ( uint8_t Port_index, const mavlink_message_t* msg )  //T265
{
		static bool vision_pos_present=false;
    ...
    const mavlink_vision_speed_estimate_t* msg_rd = (mavlink_vision_speed_estimate_t*)msg->payload64;
		if(!vision_pos_present)
		{
			bool res = PositionSensorRegister( default_vision_speed_sensor_index , \
																			Position_Sensor_Type_RelativePositioning , \
		...
		else
		{
				...
				PositionSensorUpdateVel(default_vision_speed_sensor_index,vel_vision,true);
		}
...
static void Msg84_SET_POSITION_TARGET_LOCAL_NED ( uint8_t Port_index , const mavlink_message_t* msg )
{
	...
	SendMsgToMode_Pos( mode_msg_pos, 0.01 );
}
