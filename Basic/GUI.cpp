...
static void GUI_Server(void* pvParameters)
{
	...
	DrawFont24x12( "YYY_0", 205, 90, 0x0 );
		...
		/*位置传感器状态*/
			...
			for( uint8_t i = 1; i < 8; ++i )
			{
				...
				Position_Sensor sensor;
					if( GetPositionSensor( 11, &sensor )    )
				{
					if( sensor.available  )
						DrawFont16x8( "\x1d", 208, 17+0*8, WHITE );
					else
						DrawFont16x8( "\x1e", 208, 17+0*8, WHITE );
				}
				else
					DrawFont16x8( "\x1f", 208, 17+0*8, WHITE );
