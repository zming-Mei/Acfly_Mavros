	...
	bool getReceiver( Receiver* rc, SName* name, double TIMEOUT )
	{
		...
		while( it != Receivers.end() )
		{
			if( xSemaphoreTake(it->second.mutex,TIMEOUT_Ticks) )
			{
				...
				if(it->first!=SName("Virtual")) //跳过虚拟遥控器
				{
					*rc = it->second.rc;
					if( name != 0 )
						*name = it->first;
		...
		//无接收机可用
		//更新rc_it指针
		map<SName, ReceiverSync>::iterator it_virtual = Receivers.find(SName("Virtual"));
		if(it_virtual != Receivers.end())
		{
			if( xSemaphoreTake(it_virtual->second.mutex,TIMEOUT_Ticks) )
			{
				if(it_virtual->second.rc.connected == true)//若虚拟遥控器存在则使用虚拟遥控器
				{
					*rc = it_virtual->second.rc;
					if( name != 0 )
						*name = it_virtual->first;
					xSemaphoreGive(it_virtual->second.mutex);
					if( xSemaphoreTake( rc_it_Mutex, TIMEOUT_Ticks ) )
					{
						rc_it = it_virtual;
						xSemaphoreGive(rc_it_Mutex);
