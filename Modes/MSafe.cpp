...
static void MSafe_Server(void* pvParameters)
{
	...
	while(1)
	{
		...
		bool Msafe_enable;
		get_Msafe_en(&Msafe_enable);
		if(( lowPowerState>0 || lastXYCtrlTime.get_pass_time()>1 || lastZCtrlTime.get_pass_time()>1 )&& Msafe_enable)
		{	//低电量或控制超时
			//强制进入MSafe控制
			ForceMSafeCtrl = true;			
			//打开高度控制器
...
RtLocal:
			...
			if( inFlight==false&&(!UseRcCtrl||rc.data[0]<30) )
			{
