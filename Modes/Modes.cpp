...
static QueueHandle_t message_queue = xQueueCreate( 50, sizeof(ModeMsg) );
static QueueHandle_t message_queue_pos = xQueueCreate( 50, sizeof(ModeMsg_Pos) );
			...
			switch( msg.cmd )
			{
				...
				{	//do set mode
					...
					{	//mavlink定义模式
						...
						else if( t_mav_mode.main_mode==PX4_CUSTOM_MAIN_MODE_OFFBOARD && p_mode==35)//lock the offboard to M35 only
						{
							msg_handled = true;
...
//takeoff command without arming is dangerous				
//				case 22:
//				{	//takeoff起飞
//					msg_handled = true;
